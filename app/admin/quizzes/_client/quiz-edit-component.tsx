...
import { Award, CheckCircle2, ClipboardCheck, FileQuestion, Layers3, Loader2, Pencil, Plus, Save, ShieldCheck, Trash2 } from "lucide-react"
...
export default function QuizzesEditComponent({quizId} : {quizId: string}) {
        ...
        function load() {
            safeCall(async () => {
                ...
                form.reset({
                   ...
                   interviewQuizQuestions: result.quizQuestions && result.quizQuestions.length > 0
                     ? result.quizQuestions.map(quizQuestion => 
                        ({questionTitle: quizQuestion.questionTitle,
                          questionType: quizQuestion.questionType,
                          marks: quizQuestion.marks.toString(),
                          interviewQuizOptions:  quizQuestion.quizOptions && quizQuestion.quizOptions.length > 0 ?
                            quizQuestion.quizOptions.map(quizOption => ({
                                    optionAnswer: quizOption.optionAnswer,
    ...
    async function save(quizForm : QuizForm) {
         ...
         await safeCall(async () => {
             const result = await Quiz.editQuizzes(quizId, QuizPayload(quizForm))

            toast.success("Quiz is edited", {
                description: `Quiz #${result.id} has been edited successfully.`,
            })
    ...
    return (
        <section className="mx-auto max-w-7xl space-y-6 px-1 pb-8 text-zinc-950">
            <PageTitle icon="BookOpenText" title=" Edit Quiz" description="Build interview quizzes with clear questions, structured answers, and a passing score." />
